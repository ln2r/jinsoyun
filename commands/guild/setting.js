...
module.exports = class GuildSettingsCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      group: 'guild',
      memberName: 'setting',
      description: 'Change the bot setting. Use `show` to see the current guild/server settings. To see what setting is available use `setting`',
  ...
  async run(msg, args) {
    ...
    const subQuery = ['reset', 'twitter', 'koldrak', 'welcome', 'join', 'show', 'admin', 'hunter'];
    let msgData = '';
    let embedData;
    // checking permission
    if (await utils.getAuthorPermission(msg, msg.guild.id)) {
      ...
      let settingAdminRoleText = '*No Role Selected*';

      // welcome 
      let welcomeSettings;
      let settingWelcomeMessage;
      let settingWelcomeChannelText = '*No Channel Selected*';
      let settingWelcomeMessageText = '*No Message Selected*';
      
      // join
      let joinSettings;
      let joinSettingsMessage;
      let joinSettingsMessageText = '*No Message Selected*';
      let joinSettingsRoleText = '*No Role Selected*';
      let joinSettingsChannelText = '*No Channel Selected*';
      let joinSettingsStatusText = '*Disabled*';
      ...
      if (subQuery.includes(setting)) {
        ...
        case 'join':
          ...
          case 'msg':
            ...
            if (joinSettingsMessage.length > 0) {
              if (joinSettingsMessage === 'disable') {
                joinSettings.message = null;
              } else {
                joinSettings.message = joinSettingsMessage;
                joinSettingsMessageText = joinSettingsMessage;
              }
              changed = true;
            }
          ...
          case 'ch':
            if(query[2] == 'disable'){
              ...
              joinSettingsChannelText = 'disable';          
              changed = true;
            }else{
              joinSettings.channel = utils.getChannelId(query[2]);
              joinSettingsChannelText = '<#'+utils.getChannelId(query[2])+'>';          
              changed = true;
            }         
          ...
          case 'role':
            if(query[2] !== 'disable'){
              ...
              joinSettingsRoleText = '<@&'+utils.getRoleId(query[2])+'>';          
              changed = true;
            }            
          ...
          if (!changed) {
            ...
            if (guildSettings.join.channel) joinSettingsChannelText = '<#'+guildSettings.join.next+'>';
            if (guildSettings.join.role) joinSettingsRoleText = '<@&'+guildSettings.join.role_id+'>';
          }else{
            // update the database
        ...
        case 'show':
          ...
          optionDescription = `List of current guild's settings, use \`${botPrefix}setting\` to see the options.`;
          optionEmbedData = [
            {
              'name': 'Reset',
              'value': settingResetChannelText,
            ...
            {
              'name': 'Join Command',
              'value': `Status: ${joinSettingsStatusText}\n Channel: ${joinSettingsChannelText}\nMember Role: ${joinSettingsRoleText}\nMessage:\n${joinSettingsMessageText}`,
            }            
          ];
          break;
